...
const AdminFotoGalareyaCom = () => {
  ...
  const fechtData = async () => {
    try {
      const response = await APIFotoGalareya.getFotoGalareya();
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, onSubmitProps) => {
      ...
      try {
        // POST
        if (!edit) {
          await APIFotoGalareya.postFotoGalareya(data);
        }
        // PATCH
        else {
          await APIFotoGalareya.patchFotoGalareya(id, data);
          console.log(data);
          setEdit(false);
          setId(null);
        }
  ...
  const handleDelete = async (id) => {
    try {
      await APIFotoGalareya.deleteFotoGalareya(id);
      fechtData();
    } catch (error) {
      console.error("Xatolik yuz berdi!", error);
    }
...
export default AdminFotoGalareyaCom;
