...
const AdminBolimCom = () => {
  ...
  const fechtData = async () => {
    try {
      const response = await APIBolim.getBolim();
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, onSubmitProps) => {
      ...
      try {
        // POST
        if (!edit) {
          await APIBolim.postBolim(data);
        }
        // PATCH
        else {
          await APIBolim.patchBolim(id, data);
          console.log(data);
          setEdit(false);
          setId(null);
        }
